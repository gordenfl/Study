...
5. 智能指针 std::shared_ptr, std::unique_ptr, std::weak_ptr
    ...
    * unique_ptr 自动析构,无需delete, shared_ptr最后一个引用消失的时候才析构,不过要警惕循环引用,需要配合weak_ptr解决.
    ...
    如果把上面的std::shared_ptr<B> b_ptr;  改成std::weak_ptr<B> b_ptr; 就可以避免循环引用

    * weak_ptr: 是一个弱引用的指针可以通过lock来生成shared_ptr, 这样可以判断这个指针是否还存在,如果存在就可以用,如果不存在就去另外的逻辑.

6. C++ Polymorphism(多态性) implement virtual implement
...
7. abstract virtual class and interface, virtual function
...
11. SpinLock, Mutex, Semaphore


12. Deadlock, Race Conditions
...
16. Frame, VSync (渲染管线基础)

17. Entity-Component-System 架构

18. Git版本控制 如何解决多人写作时候的合并冲突

19. 单元测试/调试 TDD思路

20. 架构设计, 设计一个模块让他可以在多人协作下容易扩展
